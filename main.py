...
def main():
    ...
    # optimization
    # https://www.stat.cmu.edu/~cshalizi/402/lectures/11-splines/lecture-11.pdf
    # https://mfe.baruch.cuny.edu/wp-content/uploads/2019/12/IRC_Lecture1_2019.pdf
    def goal(x):
        """ objective function """
        oiscoef = x[0:18]
        liborcoef = x[18:36]
        ...
        for i in range(4,10):
            for j in range(4,10):
                spline.splcrsint(i,j,1,30)
        #Tikhonov regularization
        lam = 0.0001 #series w.r.t. t
        penalty = lam * sum([spline.splcrsint(k,l,1,30) * (oiscoef[k]*oiscoef[l] + liborcoef[k]*liborcoef[l]) for k in range(4,10) for l in range(4,10)])
        return f_ret + penalty

    def min_bfgs(ls_init):
        #res = opt.fmin_bfgs(goal, ls_init, gtol = 1e-6, epsilon = 1e-6)
        res = opt.fmin_bfgs(goal, ls_init, maxiter=50)
        xopt = res
        Qmin = goal(xopt)
        return xopt
    ...
    L = np.linspace(0.1,2,20,endpoint=True)
    liborFwdRates = [libor.forwards(L[i],L[i]+0.25,spline) for i in range(19)]

    print ("Step 4: Calculating Instantaneous rate...")
    IL = np.linspace(0.01,30,3000,endpoint=True)
    instantLibor = [libor.r(IL[i],spline) for i in range(2999)]
    instantOIS = [ois.r(IL[i],spline) for i in range(2999)]
    instantBasis = [instantLibor[i]-instantOIS[i] for i in range(2999)]
    ...
    axis3.plot(np.asarray(L[:-1]), 100*np.asarray(liborFwdRates), 'b-')
    axis3.set_title("LIBOR Rates")
    
    # Instant Rate Curve

    axis4 = figure1.add_subplot(414) 
    axis4.plot(np.asarray(IL[:-1]), 100*np.asarray(instantLibor), 'r-')
    axis4.plot(np.asarray(IL[:-1]), 100*np.asarray(instantOIS), 'b-')
    axis4.plot(np.asarray(IL[:-1]), 100*np.asarray(instantBasis), 'y-')
