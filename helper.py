...
def read_DataSheetCurve(s_type):
    ...
    if s_type == 'LIBOR':
        return pd.read_excel(s_filename, s_sheetname, skiprows=1, usecols='B:E')[:2]
    elif s_type == 'ED Futures':
        return pd.read_excel(s_filename, s_sheetname, skiprows=5,  usecols='B:F')[:8]
    elif s_type == 'Swap Rates':
        return pd.read_excel(s_filename, s_sheetname, skiprows=15, usecols='B:E')[:11]
    elif s_type == 'Fed Funds':
        return pd.read_excel(s_filename, s_sheetname, skiprows=28, usecols='B:E')[:1]
    elif s_type == 'Basis Swap Rates':
        return pd.read_excel(s_filename, s_sheetname, skiprows=32, usecols='B:E')[:16]
    else:
